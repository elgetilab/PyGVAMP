#!/bin/bash
# ===========================================================================
# PyGVAMP — Ab42 Reduced: Reversible (RevGraphVAMP)  |  SLURM Array Job
# ===========================================================================
# Runs N_RUNS independent pipeline executions per lag time for the
# reduced Ab42 dataset using the reversible likelihood-based loss.
#
# Usage:
#   # 5 lag times x 5 runs = 25 tasks (indices 0-24)
#   sbatch --array=0-24 ab42_red_reversible.sh
#
#   # Limit to 2 concurrent GPU jobs
#   sbatch --array=0-24%2 ab42_red_reversible.sh
# ===========================================================================

# ---- SLURM directives -----------------------------------------------------
#SBATCH --job-name=ab42r_rev
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/ab42r_rev_%A_%a.out
#SBATCH --error=logs/ab42r_rev_%A_%a.err
...
# ---- Print job info --------------------------------------------------------
echo "============================================================"
echo "PyGVAMP — Ab42 Reduced, Reversible (RevGraphVAMP)"
echo "============================================================"
