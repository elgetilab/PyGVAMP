#!/usr/bin/env python3
"""
Aggregate the v11 10-seed array into cross-seed statistics for both
...
"3.78 ± 0.02 averaged across 10 different trainings".

Usage:
  python cluster_scripts/aggregate_villin_v11_array.py
  python cluster_scripts/aggregate_villin_v11_array.py --root /mnt/hdd/experiments/villin_repro_v11
  python cluster_scripts/aggregate_villin_v11_array.py --csv summary.csv
"""
...
# Matches: "New best model with score: 3.7293"
...
class SeedResult:
    ...
    best_concat: float
    best_epoch: int
    perbatch_mean: float  # at best_epoch
    perbatch_std: float   # at best_epoch
    final_epoch: int      # last epoch reached (100 if completed)
    completed: bool       # True if "Loaded best model with score" found
...
def latest_log(seed_dir: Path) -> Optional[Path]:
    ...
    Layout:
      seed_dir/exp_villin_<TIMESTAMP>/logs/log_<TIMESTAMP>.txt
    """
    candidates = sorted(seed_dir.glob("exp_villin_*/logs/log_*.txt"))
    return candidates[-1] if candidates else None


def parse_log(log_path: Path, seed: int) -> Optional[SeedResult]:
    """Parse a v11 training log for the canonical (concat, perbatch) at
    the best-concat epoch.  Returns None if the log can't be parsed
    (e.g. crashed before training started)."""
    with log_path.open() as f:
        lines = f.readlines()

    # First pass: collect every (epoch, train, concat, perbatch_mean, perbatch_std)
    # from "Epoch K/100, ..." lines.
    epoch_data: dict[int, tuple[float, float, float]] = {}  # epoch -> (concat, pm, ps)
    final_epoch = 0
    for line in lines:
    ...
    # selection rule in vampnet.py is "strict >", so an exact match
    # corresponds to the moment best_concat was set.
    best_epoch = None
    ...
    if best_epoch is None:
        # Fallback: epoch with highest concat
...
def print_summary(results: list[SeedResult], paper_mean=3.78,
    ...
    # Tail statistics — useful for "did any single seed match the paper?"
...
def main():
    ...
    parser.add_argument("--root", type=Path,
                        default=Path("/mnt/hdd/experiments/villin_repro_v11"),
    ...
    parser.add_argument("--paper-mean", type=float, default=3.78,
                        help="Paper-reported mean for comparison (default 3.78).")
    parser.add_argument("--paper-sigma", type=float, default=0.02,
                        help="Paper-reported sigma for comparison (default 0.02).")
