#!/usr/bin/env python3
"""
Aggregate the NTL9 v1 10-seed array into cross-seed statistics for
...
"VAMP-2 = 4.59 ± 0.09 averaged across 10 different trainings" (Table S1).

Usage:
  python cluster_scripts/aggregate_ntl9_v1_array.py
  python cluster_scripts/aggregate_ntl9_v1_array.py --root /mnt/hdd/experiments/ntl9_repro_v1
  python cluster_scripts/aggregate_ntl9_v1_array.py --csv summary.csv
"""
...
def latest_log(seed_dir: Path) -> Optional[Path]:
    ...
    Layout:
      seed_dir/exp_ntl9_<TIMESTAMP>/logs/log_<TIMESTAMP>.txt
    """
    candidates = sorted(seed_dir.glob("exp_ntl9_*/logs/log_*.txt"))
...
def print_summary(results: list[SeedResult], paper_mean=4.59,
                  paper_sigma=0.09) -> None:
...
def main():
    ...
    parser.add_argument("--root", type=Path,
                        default=Path("/mnt/hdd/experiments/ntl9_repro_v1"),
    ...
    parser.add_argument("--paper-mean", type=float, default=4.59,
                        help="Paper-reported mean for comparison (default 4.59).")
    parser.add_argument("--paper-sigma", type=float, default=0.09,
                        help="Paper-reported sigma for comparison (default 0.09).")
